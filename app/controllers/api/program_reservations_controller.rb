...
class Api::ProgramReservationsController < ApiController
  ...
  def create
    program_create_service = Programs::CreateService.new(program_reservation_params)
    @program_reservation = program_create_service.perform
  ...
  def cancel
    @program_reservation.canceled_at = Date.current
  ...
  def validate_cancellation
    render json: { alert: "Program reservation was canceled." } if @program_reservation.canceled_at.present?
  end
